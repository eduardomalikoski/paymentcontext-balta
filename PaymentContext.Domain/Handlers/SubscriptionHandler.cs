...
namespace PaymentContext.Domain.Handlers
{
    public class SubscriptionHandler : Notifiable, IHandler<CreateBoletoSubscriptionCommand>, IHandler<CreatePayPalSubscriptionCommand>, IHandler<CreateCreditCardSubscriptionCommand>
    {
        ...
        public ICommandResult Handle(CreateBoletoSubscriptionCommand command)
        {
            ...
            _emailService.Send(student.Name.ToString(), student.Email.Address, "Bem vindo amigo", "Sua assinatura foi criada");
        ...
        public ICommandResult Handle(CreateCreditCardSubscriptionCommand command)
        {
            ...
            var student = new Student(name, document, email);
            var subscription = new Subscription(DateTime.Now.AddMonths(1));
            var payment = new CreditCardPayment(command.CardNumber, command.CardHolderName, command.LastTransactionNumber, command.PaidDate, command.ExpireDate, command.Total, command.TotalPaid, command.Payer,
                            new Document(command.PayerDocument, command.PayerDocumentType), address, email);
